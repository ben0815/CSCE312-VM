...
TranslateWriteFunction(std::string _funct) {
    ...
    _funct = _funct.substr(fname.size(), _funct.size());
...
TranslateCallFunction(std::string _call) {
    ...
    std::string numArgs = _call.substr(fname.size(), _call.size());
    std::string returnLabel = fname + "_RETURN_" + std::to_string(return_counter);
    return_counter += 1;
